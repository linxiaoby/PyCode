...
def load_data(data_dir, max_doc_length=10, max_sent_length=50):
    ...
    word_vocab.feed(' ')
    word_vocab.feed('{')
    word_vocab.feed('}') # word mapped to index '':0,'{':1, '}':2

    actual_max_doc_length = 0

    word_tokens = collections.defaultdict(list) # a new dict
    ...
    for fname in ('train', 'valid', 'test'):
        ...
        for dname in os.listdir(pname):
            ...
            with codecs.open(os.path.join(pname, dname), 'r', 'utf-8') as f: #codecs used for decode transfer
                lines = f.read().split('\n\n')  # lines are the whole article
                word_doc = []
                label_doc = []
                sent_list = lines[0].split('\n')
                sent_num = len(sent_list)
                i = 0
                for i in range(sent_num):
                    if '\t\t\t' not in sent_list[i]: #the lines without label should be dropped
                        continue
                    line = sent_list[i].strip()
                    line = line.replace('}', '').replace('{', '').replace('|', '')
                    line = line.replace('<unk>', ' | ')

                    sent, label = line.split('\t\t\t') # sentence + label ([0, 1,2])
                    label_doc.append(label)
                    sent = sent.split(' ') #get words lists of a sentence
                    ...
                    word_doc.append(word_array) #word_doc are word index
                ...
                word_tokens[fname].append(word_doc) #word_doc : words of an article
                labels[fname].append(label_doc)

    assert actual_max_doc_length <= max_doc_length # max sentence num of an article
    ...
    label_tensors = {}
    for fname in ('train', 'valid', 'test'):
        word_tensors[fname] = np.zeros([len(word_tokens[fname]), actual_max_doc_length, max_sent_length], #input to convol layer
                                       dtype=np.int32)
